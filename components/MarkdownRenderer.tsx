...
const MarkdownRenderer: React.FC<Props> = ({ content }) => {
  return (
    <div className="prose prose-invert max-w-none prose-pre:bg-slate-900 prose-pre:p-0 prose-code:text-blue-300">
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        components={{
          code({ node, className, children, ...props }: any) {
            const match = /language-(\w+)/.exec(className || '');
            // In react-markdown v9/v10, block code has a language class, 
            // while inline code typically does not.
            return match ? (
